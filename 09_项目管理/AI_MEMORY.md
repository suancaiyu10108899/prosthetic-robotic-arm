# AI 记忆管理文件

> **用途**：本文件帮助 AI 助手在每次对话中快速了解项目全貌，实现持续跟进。
> **维护规则**：每次与 AI 协作完成重要工作后，更新本文件对应的部分。
> **最后更新**：2026-06-11

---

## 1. 项目基本信息

| 项目 | 内容 |
|------|------|
| **项目名称** | 假肢机械臂 |
| **项目路径** | `d:\假肢机械臂\`（文档归档） + `D:\Dev\arm-ble\`（代码开发） |
| **GitHub** | https://github.com/suancaiyu10108899/prosthetic-robotic-arm (Private) |
| **负责人** | (你的名字 / 年级：大二下) |
| **实验室** | (实验室名称) |
| **开始时间** | 2026年6月 |
| **当前阶段** | ✅ 板① Blink 烧录验证通过（2026-06-11），待获取遥控器协议后编写蓝牙固件 |

## 2. 当前任务背景

- 老师原有机械臂被**剧组借用改造**
- 需要用**未来工场8200pro树脂**做光固化打印
- 同时有一**另一只手**（合作方设计）的零件也要打印
- 另一只手包含：接受腔、尼龙件（左盖/把/箍/连杆/销）、树脂件（E-1/E-2）

### 电控系统架构（已确认）

```
蓝牙信号源 (手机/手柄)
    │ BLE
    ▼
┌─ 电池模块 (电池箱) ────────────────┐
│  ① Adafruit Feather nRF52840 Express   │  ← 🔴 当前任务：待烧录验证
│     (nRF52840, MicroUSB 供电+编程)     │
│  ② 电压转换板                         │  ← 纯硬件，无需编程
└──────────┬──────────────────────────┘
           │ 有线信号 (UART/SPI/I2C)
           ▼
┌─ 机械臂控制板 ──────────────────────┐
│  ③ STM32/cybathlon   ← 发PWM        │  ← 用塔克DAP无线调试
└────────────────────────────────────┘
```

- **板①**：**Adafruit Feather nRF52840 Express**（nRF52840 芯片，已实物确认丝印，✅ 已烧录 Blink 验证通过）
- **板②**：电压转换，纯硬件，无需编程
- **板③**：STM32 控制板，用塔克 DAP（ESP32-S3, 30m）无线调试（在学长处）

### 开发环境

| 项目 | 路径 | 工具 |
|------|------|------|
| 文档管理仓库 | `d:\假肢机械臂\` | Git + GitHub |
| 固件开发工作区 | `D:\Dev\arm-ble\` | PlatformIO 6.1.19 (pip) |
| Blink 测试 | ✅ 编译成功 | `src/main.cpp` |
| 原固件备份 | `src/sketch_feb3a.bak` | 待恢复为遥控版 |

> PlatformIO 命令入口：`python -m platformio run --project-dir=d:\Dev\arm-ble --target upload`

## 3. 目录结构（当前）

```
d:\假肢机械臂\
├── 00_原始压缩包归档/         # 电池盒/主控板压缩包备份
├── 01_机械结构/
│   ├── 原版机械臂/                # 老师原有设计
│   │   ├── SolidWorks/            # SW源文件（零件/装配体/图纸）
│   │   ├── STL_打印件/            # 树脂件_8200pro / 尼龙件
│   │   ├── STEP文件/              # 【打印件】走线辅助卡口.STEP
│   │   ├── 通用件_电池盒/         # 电池盒（属于原版机械臂）
│   │   └── 设计笔记/
│   └── 另一只手/                  # 合作方设计
│       ├── 接受腔/、STL_尼龙打印件/、STL_树脂打印件/
│       └── 设计笔记/
├── 02_电子与控制/
│   ├── 系统架构说明.md           ← 电控全貌（已更新）
│   ├── 元器件采购清单.md         ← 已购+待购
│   ├── 电池模块/Arduino蓝牙板/   ← 空（待放实物照片）
│   ├── 电池模块/电压转换板/      ← 纯硬件
│   ├── 机械臂控制板/             ← STM32（待填）
│   ├── 无线调试器_塔克DAP/       ← DAP手册已归档
│   └── 主控板_cybathlon/         ← Altium PCB+原理图
├── 03_嵌入式代码/
│   └── 原版机械臂_Arduino/
│       └── sketch_feb3a.ino       # 原固件存档
├── 04_上位机/、05_实验数据/
├── 06_文献与参考资料/
│   ├── 塔克_DAP调试器手册_V2.0.pdf
│   └── 其他PDF参考资料
├── 07_学习笔记/                  # 3篇已完成
├── 08_周报与汇报/
├── 09_项目管理/                  # README / AI_MEMORY / 任务看板 / 问题追踪
└── .gitignore
```

## 4. 重要约定与规则

1. **STL/SolidWorks 大文件**：不入 Git（`.gitignore` 已配置），AI 不读取内容
2. **文件命名规范**：
   - 你自己画的零件：`【打印件/加工件/铝方管】零件名.后缀`
   - 老师/他人发来的文件：保留原名
   - 版本号：加 `_V2`、`_V3` 后缀
3. **打印材料**：未来工场8200pro树脂（光固化）+ 尼龙件
4. **开发代码与文档分离**：代码在 `D:\Dev\arm-ble\`（纯英文路径），文档在 `d:\假肢机械臂\`
5. **Git 只追踪文档和代码，不追踪大文件**

## 5. 现有文件清单

| 模块 | 文件 | 类型 |
|------|------|------|
| 原版机械臂 | 【打印件】机械臂盖板(.STL/.SLDPRT) ×2 | STL/SW |
| 原版机械臂 | 【打印件】走线辅助卡口(.SLDPRT/.STEP) | SW/STEP |
| 原版机械臂 | 内腔(.stl/.SLDPRT) ×3 | STL树脂 |
| 原版机械臂 | 外腔_敏姐_小周改(.stl/.SLDPRT) ×3 | STL树脂 |
| 原版机械臂 | 圆孔.stl | STL树脂 |
| 原版机械臂 | 盖板装配.SLDASM | SW装配 |
| 另一只手 | 接受腔/尼龙件(6个)/树脂件(2个) | STL |
| 通用件 | 电池盒 SW零件×5 + 装配×1 + STL×4 | SW/STL |
| 电子 | cybathlon Altium工程 | PCB+原理图 |
| 电子 | 系统架构说明 + 元器件采购清单 | 文档 |
| 电子 | 塔克 DAP 手册 | PDF |
| 嵌入式 | sketch_feb3a.ino（固件存档） | Arduino |
| 开发环境 | `D:\Dev\arm-ble\` PlatformIO 项目 | 可编译 |
| 学习笔记 | 3篇（BLE/滤波/传感器） + 开发日志1篇 | 文档 |

## 6. 技术栈

| 领域 | 技术/工具 | 状态 |
|------|-----------|------|
| 机械设计 | SolidWorks | ✅ 已有 |
| 3D打印材料 | 未来8200pro树脂 + 尼龙 | 待打印 |
| PCB设计 | Altium Designer (cybathlon) | ✅ 已有 |
| 嵌入式-BLE | PlatformIO + nRF52840 (Adafruit Feather) | ✅ 烧录验证通过 (COM7 Blink OK) |
| 嵌入式-主控 | STM32U575 (ARM Cortex-M33) | ❓ 源码缺失 |
| 传感器-角度 | Bend Labs 弯曲传感器 (BLE, ±250°) | ✅ 固件已有 |
| 传感器-生物 | EMG 双通道肌电信号 | ⚠️ PCB预留 |
| 无线调试 | 塔克 DAP (ESP32-S3, 30m, SWD/JTAG) | ✅ 手册已归档 |
| 上位机 | - | 待开始 |

## 7. 关键决策记录

| 日期 | 决策内容 | 原因 |
|------|----------|------|
| 2026-06-10 | 大文件不入 Git | STL/SW太大(300MB) |
| 2026-06-10 | 目录按原版/另一只手拆分 | 两个不同设计 |
| 2026-06-10 | 你的零件加【打印件】前缀 | 区分来源 |
| 2026-06-10 | 压缩包统一归档 | 避免散落 |
| 2026-06-11 | 确认板型：Adafruit Feather nRF52840 Express | 实物丝印确认 |
| 2026-06-11 | 代码在 D:\Dev\arm-ble\（纯英文）| 避免中文路径编译问题 |
| 2026-06-11 | PlatformIO 6.1.19 通过 pip 安装 | VSCode插件CLI破损 |

## 8. 老师发的电控元件

| 元件 | 用途 |
|------|------|
| GX12-3芯航空插头 + 连接线 | 机械臂模块间电源/信号 |
| Adafruit nRF52840 Express | 蓝牙板（已实物确认） |
| BLE ESP32遥控手柄 | 无线遥控（待购） |
| CodexPad ESP32-S3手柄 | 可编程手柄替代方案 |

## 9. 待解决问题

- 另一只手的具体身份？
- 剧组具体的改造需求？
- STM32U575 固件源码在哪里？
- 300MB扫描模型具体路径？（AI不读取，但需知道位置）
- EMG 传感器模组是否已有实物？

## 10. 学习笔记索引

| 笔记 | 路径 | 内容 |
|------|------|------|
| 笔记1 | `07_学习笔记/嵌入式开发/nRF52_BLE传感器与舵机控制_20260610.md` | BLE通信、舵机映射、标定协议 |
| 笔记2 | `07_学习笔记/控制算法/IIR滤波与死区滤波_20260610.md` | IIR低通(20Hz)、死区(0.5°) |
| 笔记3 | `07_学习笔记/传感器与信号处理/BendLabs弯曲传感器与EMG肌电控制_20260610.md` | 传感器原理、融合方案 |
| 日志1 | `D:\Dev\arm-ble\docs\devlog\2026-06-11_环境搭建.md` | 开发环境搭建纪要 |

---

> 📌 **提示**：与 AI 协作完成以下操作后记得更新本文件：
> - 新增/修改目录结构
> - 确定技术选型
> - 完成重要里程碑
> - 做出关键决策