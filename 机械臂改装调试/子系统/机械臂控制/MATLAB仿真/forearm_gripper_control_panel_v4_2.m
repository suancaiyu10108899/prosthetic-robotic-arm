function forearm_gripper_control_panel_v4_2
% FOREARM_GRIPPER_CONTROL_PANEL_V4_2
...
ikGrid.RowHeight = {165,255,230,130,'1x'}; ikGrid.Padding=[6 6 6 6]; ikGrid.RowSpacing = 6;
...
gButtons = uigridlayout(pButtons,[5 2]);
gButtons.RowHeight = {48,48,48,48,48};
gButtons.ColumnWidth = {'1x','1x'};
gButtons.Padding = [10 6 10 6];
gButtons.RowSpacing = 4;
gButtons.ColumnSpacing = 10;
uibutton(gButtons,'Text','读取当前TCP','ButtonPushedFcn',@(~,~)readCurrentTCP(), 'FontSize',13);
uibutton(gButtons,'Text','预览目标点','ButtonPushedFcn',@(~,~)previewTarget(), 'FontSize',13);
uibutton(gButtons,'Text','解析位置IK','ButtonPushedFcn',@(~,~)solveIK(), 'FontSize',13);
uibutton(gButtons,'Text','应用推荐解','ButtonPushedFcn',@(~,~)applyIK(1), 'FontSize',13);

% Row 3: 应用指定编号
gSub3 = uigridlayout(gButtons,[1 3]);
gSub3.ColumnWidth = {'1x',60,'1x'};
gSub3.Padding = [0 0 0 0];
gSub3.ColumnSpacing = 4;
applyIndex = uieditfield(gSub3,'numeric','Value',1,'Limits',[1 Inf],'RoundFractionalValues','on','FontSize',13);
uibutton(gSub3,'Text','应用#','ButtonPushedFcn',@(~,~)applyIK(applyIndex.Value), 'FontSize',13);
utilLabelForRow3 = uilabel(gSub3,'Text','');

uibutton(gButtons,'Text','清除目标/投影','ButtonPushedFcn',@(~,~)clearIKGraphics(), 'FontSize',13);
uibutton(gButtons,'Text','帮助说明','ButtonPushedFcn',@(~,~)showIKHelp(), 'FontSize',13);

pNudge = uipanel(ikGrid,'Title','微调 / 拖动目标点','FontWeight','bold');
gNudge = uigridlayout(pNudge,[5 4]);
gNudge.RowHeight = {42,42,42,42,42};
gNudge.ColumnWidth = {85,80,90,'1x'};
gNudge.Padding = [10 6 10 6];
gNudge.RowSpacing = 4;
gNudge.ColumnSpacing = 8;

% Row1: 步长 + X微调
uilabel(gNudge,'Text','步长 [mm]','FontSize',12);
nudgeStep = uieditfield(gNudge,'numeric','Value',5,'Limits',[0.01 100],'ValueDisplayFormat','%.2f','FontSize',13);
uibutton(gNudge,'Text','X -','ButtonPushedFcn',@(~,~)nudgeTarget(1,-1), 'FontSize',13);
uibutton(gNudge,'Text','X +','ButtonPushedFcn',@(~,~)nudgeTarget(1,+1), 'FontSize',13);

% Row2: 微调勾选 + Y微调
cbNudgeSolve = uicheckbox(gNudge,'Text','微调自动求解','Value',true,'FontSize',12);
cbNudgeApply = uicheckbox(gNudge,'Text','微调自动应用','Value',false,'FontSize',12);
uibutton(gNudge,'Text','Y -','ButtonPushedFcn',@(~,~)nudgeTarget(2,-1), 'FontSize',13);
uibutton(gNudge,'Text','Y +','ButtonPushedFcn',@(~,~)nudgeTarget(2,+1), 'FontSize',13);

% Row3: 拖动平面 + Z微调
uilabel(gNudge,'Text','拖动平面','FontSize',12);
ddDragPlane = uidropdown(gNudge,'Items',{'XY','XZ','YZ'},'Value','XZ','FontSize',12);
uibutton(gNudge,'Text','Z -','ButtonPushedFcn',@(~,~)nudgeTarget(3,-1), 'FontSize',13);
uibutton(gNudge,'Text','Z +','ButtonPushedFcn',@(~,~)nudgeTarget(3,+1), 'FontSize',13);

% Row4: 拖动开关
cbDragEnable = uicheckbox(gNudge,'Text','启用鼠标拖动','Value',false,'FontSize',12);
cbDragSolve = uicheckbox(gNudge,'Text','拖动自动求解','Value',true,'FontSize',12);
cbDragApply = uicheckbox(gNudge,'Text','拖动自动应用','Value',false,'FontSize',12);
uilabel(gNudge,'Text','','FontSize',12);

% Row5: 拖动约束
uilabel(gNudge,'Text','拖动约束','FontSize',12);
ddDragConstraint = uidropdown(gNudge,'Items',{'无约束','固定g球面','自动g球壳'},'Value','自动g球壳','FontSize',12);
uilabel(gNudge,'Text','','FontSize',12);
uilabel(gNudge,'Text','','FontSize',12);
...
ikTable = uitable(ikGrid,'ColumnName',{'#','q1','q2','q3','g','posErr','投影误差','dist'}, ...
    'ColumnWidth',{35,64,64,64,64,82,82,72});
...
ikExplain.Value = { ...
    ...
    '当前 IK 是“位置 IK + 指定 q3 滚转”，不是任意 6D 位姿 IK。'; ...
    'V4.1 增加目标微调按钮和平面鼠标拖动：勾选“启用鼠标拖动”后，在图中点击/拖动红叉或绿圈。'};
...
for i=1:numel(keys)
    ...
    hPatch.(keys{i}) = patch(ax,'Faces',M.F,'Vertices',M.V,'Parent',hT.(keys{i}), ...
        ...
        'DiffuseStrength',0.75,'SpecularStrength',0.12,'HitTest','off');
end
...
hTarget=plot3(ax,nan,nan,nan,'rx','LineWidth',2.4,'MarkerSize',13,'ButtonDownFcn',@(~,~)startTargetDrag());
hUsed=plot3(ax,nan,nan,nan,'go','LineWidth',2.0,'MarkerSize',10,'ButtonDownFcn',@(~,~)startTargetDrag());
ax.ButtonDownFcn = @(~,~)axesClickOrDrag();
...
%% ===================== 回调函数 =====================
    ...
    function nudgeTarget(axisIndex,sgn)
        p=[ikX.Value;ikY.Value;ikZ.Value];
        step = nudgeStep.Value;
        p(axisIndex)=p(axisIndex)+sgn*step;
        updateTargetFields(p,true);
        if cbNudgeSolve.Value
            solveIK();
            if cbNudgeApply.Value && ~isempty(state.lastIK.q)
                % Apply best solution
                qDegNew = rad2deg(state.lastIK.q(:,1));
                state.qDeg = qDegNew(:);
                for k=1:4, slider(k).Value=state.qDeg(k); field(k).Value=state.qDeg(k); end
                updateAll(false);
            end
    ...
    function updateTargetFields(p,showMarker)
        ikX.Value=p(1); ikY.Value=p(2); ikZ.Value=p(3);
        state.targetPoint=p;
        if showMarker
            set(hTarget,'XData',p(1),'YData',p(2),'ZData',p(3));
        end
    end
    function axesClickOrDrag()
        if ~cbDragEnable.Value, return; end
        p=mousePointOnSelectedPlane();
        if any(~isfinite(p)), return; end
        p=applyDragConstraintToPoint(p);
        updateTargetFields(p,true);
        state.usedPoint=[nan;nan;nan]; set(hUsed,'XData',nan,'YData',nan,'ZData',nan);
        if cbDragSolve.Value
            solveIK();
            if cbDragApply.Value && ~isempty(state.lastIK.q), applyIK(1); end
        end
        startTargetDrag();
    end
    function startTargetDrag()
        ...
        fig.WindowButtonUpFcn=@(~,~)stopTargetDrag();
    end
    function dragMotion()
        p=mousePointOnSelectedPlane();
        if any(~isfinite(p)), return; end
    ...
    function stopTargetDrag()
        fig.WindowButtonMotionFcn='';
        fig.WindowButtonUpFcn='';
    end
    function p=mousePointOnSelectedPlane()
        cp=ax.CurrentPoint;
        p0=cp(1,:).'; p1=cp(2,:).'; v=p1-p0;
        ref=[ikX.Value;ikY.Value;ikZ.Value];
        switch ddDragPlane.Value
            case 'XY'
                idx=3; fixed=ref(3);
            case 'XZ'
                idx=2; fixed=ref(2);
    ...
    function p=applyDragConstraintToPoint(p)
        C=[model.L12;0;0]; d=p-C; L=norm(d);
        if L<1e-9, return; end
        switch ddDragConstraint.Value
            case '固定当前g球面'
                Lc=radiusFromG(deg2rad(state.qDeg(4)),model);
                p=C+Lc*d/L;
            case '自动g球壳'
                Lmin=radiusFromG(model.qlim(4,2),model);
                Lmax=radiusFromG(model.qlim(4,1),model);
                Lc=min(max(L,Lmin),Lmax);
                p=C+Lc*d/L;
            otherwise
                % 无约束
        end
    ...
    function applyIK(idx)
        ...
        qDegNew = rad2deg(state.lastIK.q(:,idx));
        state.qDeg = qDegNew(:);
        for k=1:4, slider(k).Value=state.qDeg(k); field(k).Value=state.qDeg(k); end
        updateAll(false);
    end
    function showIKHelp()
        msg = sprintf(['当前 IK 是位置逆解：求 q1、q2、g，并使用指定 q3。\n\n', ...
            '固定 g 时 TCP 必须在对应球面上；程序会把目标点投影到该球面并显示投影误差。\n\n', ...
            '完整 6D 位姿 IK 不是任意可解，因为本机构只有 4 个主动自由度。\n\n', ...
            '拖动：勾选“启用鼠标拖动”，选择 XY/XZ/YZ 平面；可选固定g球面或自动g球壳约束。']);
        uialert(fig, msg, 'IK 说明');
    end
