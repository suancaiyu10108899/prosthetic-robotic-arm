function forearm_gripper_control_panel_v4_1
% FOREARM_GRIPPER_CONTROL_PANEL_V4_1
...
fig = uifigure('Name','前臂-腕部-夹爪 正逆运动学实验平台 V4.1', ...
    'Position',[20 20 1620 940], 'Color',[0.96 0.97 0.98]);
main = uigridlayout(fig,[1 2]);
main.ColumnWidth = {930,'1x'};
...
%% 4.2 IK 页
ikGrid = uigridlayout(tabIK,[6 1]);
ikGrid.RowHeight = {170,168,180,138,162,'1x'}; ikGrid.Padding=[6 6 6 6];
...
gButtons = uigridlayout(pButtons,[4 2]);
gButtons.RowHeight = {36,36,36,36};
gButtons.ColumnWidth = {'1x','1x'};
gButtons.Padding = [8 8 8 8];
...
pNudge = uipanel(ikGrid,'Title','目标微调 / 平面拖动','FontWeight','bold');
gNudge = uigridlayout(pNudge,[4 4]);
gNudge.RowHeight = {32,32,32,32};
gNudge.ColumnWidth = {'1x','1x','1x','1x'};
gNudge.Padding = [8 8 8 8];
uilabel(gNudge,'Text','步长 mm','HorizontalAlignment','center');
nudgeStep = uieditfield(gNudge,'numeric','Value',5,'Limits',[0.01 100],'ValueDisplayFormat','%.2f');
cbNudgeSolve = uicheckbox(gNudge,'Text','微调后自动IK','Value',true);
cbNudgeApply = uicheckbox(gNudge,'Text','微调后应用','Value',false);
uibutton(gNudge,'Text','X -','ButtonPushedFcn',@(~,~)nudgeTarget(1,-1));
uibutton(gNudge,'Text','X +','ButtonPushedFcn',@(~,~)nudgeTarget(1,+1));
uibutton(gNudge,'Text','Y -','ButtonPushedFcn',@(~,~)nudgeTarget(2,-1));
uibutton(gNudge,'Text','Y +','ButtonPushedFcn',@(~,~)nudgeTarget(2,+1));
uibutton(gNudge,'Text','Z -','ButtonPushedFcn',@(~,~)nudgeTarget(3,-1));
uibutton(gNudge,'Text','Z +','ButtonPushedFcn',@(~,~)nudgeTarget(3,+1));
uilabel(gNudge,'Text','拖动平面','HorizontalAlignment','center');
ddDragPlane = uidropdown(gNudge,'Items',{'XY 固定Z','XZ 固定Y','YZ 固定X'},'Value','XZ 固定Y');
cbDragEnable = uicheckbox(gNudge,'Text','启用鼠标拖动','Value',false);
cbDragSolve = uicheckbox(gNudge,'Text','拖动时自动IK','Value',true);
cbDragApply = uicheckbox(gNudge,'Text','拖动时应用','Value',false);
ddDragConstraint = uidropdown(gNudge,'Items',{'无约束','固定当前g球面','自动g球壳'},'Value','自动g球壳');
...
%% ===================== 回调函数 =====================
    ...
    function nudgeTarget(axisIndex,sgn)
        ...
        p(axisIndex)=p(axisIndex)+sgn*nudgeStep.Value;
        updateTargetFields(p,true);
        if cbNudgeSolve.Value
            solveIK();
            if cbNudgeApply.Value && ~isempty(state.lastIK.q), applyIK(1); end
        end
    end
